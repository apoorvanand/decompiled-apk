...
class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final float log(float f, float f2) {
        return (f2 <= 0.0f || f2 == 1.0f) ? FloatCompanionObject.INSTANCE.getNaN() : (float) (Math.log((double) f) / Math.log((double) f2));
    }
    ...
    public static final float truncate(float f) {
        ...
        return (float) (f > ((float) 0) ? Math.floor((double) f) : Math.ceil((double) f));
    }
