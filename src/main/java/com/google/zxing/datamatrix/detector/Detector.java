...
public final class Detector {
    ...
    private ResultPoint correctTopRightRectangular(ResultPoint resultPoint, ResultPoint resultPoint2, ResultPoint resultPoint3, ResultPoint resultPoint4, int i, int i2) {
        ...
        ResultPoint resultPoint6 = new ResultPoint(resultPoint4.getX() + (((resultPoint4.getX() - resultPoint2.getX()) / distance4) * distance3), resultPoint4.getY() + (distance3 * ((resultPoint4.getY() - resultPoint2.getY()) / distance4)));
        if (isValid(resultPoint5)) {
            return (isValid(resultPoint6) && Math.abs(i - transitionsBetween(resultPoint3, resultPoint5).c()) + Math.abs(i2 - transitionsBetween(resultPoint2, resultPoint5).c()) > Math.abs(i - transitionsBetween(resultPoint3, resultPoint6).c()) + Math.abs(i2 - transitionsBetween(resultPoint2, resultPoint6).c())) ? resultPoint6 : resultPoint5;
        }
        if (isValid(resultPoint6)) {
            return resultPoint6;
        }
    ...
    public DetectorResult detect() {
        ...
        increment(hashMap, resultPointsAndTransitions.a());
        increment(hashMap, resultPointsAndTransitions.b());
        increment(hashMap, resultPointsAndTransitions2.a());
        increment(hashMap, resultPointsAndTransitions2.b());
        ...
        int c = transitionsBetween(resultPoint13, resultPoint14).c();
        int c2 = transitionsBetween(resultPoint11, resultPoint14).c();
        if ((c & 1) == 1) {
            c++;
        }
        int i = c + 2;
        ...
        if (i * 4 >= i2 * 7 || i2 * 4 >= i * 7) {
            ...
            int c3 = transitionsBetween(resultPoint, resultPoint2).c();
            int c4 = transitionsBetween(resultPoint11, resultPoint2).c();
            if ((c3 & 1) == 1) {
                c3++;
            }
            int i3 = c3;
            ...
            bitMatrix = sampleGrid(this.image, resultPoint, resultPoint12, resultPoint11, resultPoint2, i3, c4);
        } else {
            ...
            int max = Math.max(transitionsBetween(resultPoint13, resultPoint2).c(), transitionsBetween(resultPoint11, resultPoint2).c()) + 1;
            if ((max & 1) == 1) {
                max++;
            }
            int i4 = max;
