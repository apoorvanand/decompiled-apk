...
public final class FirebaseAnalytics {
    ...
    public static FirebaseAnalytics getInstance(@NonNull Context context) {
        if (zzabt == null) {
            synchronized (FirebaseAnalytics.class) {
                if (zzabt == null) {
                    zzabt = zzz.zzf(context) ? new FirebaseAnalytics(zzz.zza(context)) : new FirebaseAnalytics(zzfj.zza(context, (zzx) null));
                }
            }
        }
        return zzabt;
    ...
    public final void zzbg(String str) {
        synchronized (this.zzabx) {
            this.zzabv = str;
            this.zzabw = this.zzl ? DefaultClock.getInstance().elapsedRealtime() : this.zzj.zzx().elapsedRealtime();
        }
    }
    ...
    public final String zzi() {
        synchronized (this.zzabx) {
            if (Math.abs((this.zzl ? DefaultClock.getInstance() : this.zzj.zzx()).elapsedRealtime() - this.zzabw) >= 1000) {
                return null;
            }
