...
public final class zzmy implements Handler.Callback {
    private static final Object lock = new Object();
    @GuardedBy("lock")
    private static zzmy zzaoh;
    private final Handler handler;

    private zzmy(Looper looper) {
    ...
    public static zzmy zzki() {
        zzmy zzmy;
        synchronized (lock) {
            if (zzaoh == null) {
                HandlerThread handlerThread = new HandlerThread("FirebaseMLHandler", 9);
                handlerThread.start();
                zzaoh = new zzmy(handlerThread.getLooper());
            }
            zzmy = zzaoh;
        }
        return zzmy;
    ...
    public final <ResultT> Task<ResultT> zza(Callable<ResultT> callable) {
        ...
        this.handler.post(new zzmz(this, callable, taskCompletionSource));
