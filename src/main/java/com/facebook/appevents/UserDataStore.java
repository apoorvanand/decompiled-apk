...
public class UserDataStore {
        ...
        for (String str : bundle.keySet()) {
            try {
                ...
                if (maybeSHA256Hashed(obj)) {
                    encryptData = obj.toLowerCase();
                } else {
                    encryptData = encryptData(normalizeData(str, bundle.get(str).toString()));
                    if (encryptData == null) {
                    }
                }
    ...
    public static void initAndWait() {
        if (!initialized) {
            ...
            try {
                if (!initialized) {
                    hashedUserData = PreferenceManager.getDefaultSharedPreferences(FacebookSdk.getApplicationContext()).getString(USER_DATA_KEY, (String) null);
                    initialized = true;
    ...
    public static void initStore() {
        if (!initialized) {
            AppEventsLogger.e().execute(new Runnable() {
                public void run() {
